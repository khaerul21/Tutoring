        ...
        <div class="m-3 flex items-center justify-center">
            <form class="flex items-center" action="{{ route('admin-manage-courses') }}">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" id="voice-search" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5" placeholder="Search Course" name="search" value="{{ request('search') }}">
                </div>
            ...
            <a href="{{ route('admin-create-course') }}" class="inline-flex items-center py-2.5 px-3 ms-2 text-sm font-medium text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">+ Add Course</a>
        </div>
    </div>
    <div class="container flex flex-wrap mx-auto mt-10 mb-10 justify-center items-center">
        @if(count($courses) > 0)
            @foreach ($courses as $course)
                <div class="max-w-sm m-10 bg-white border border-gray-200 rounded-lg shadow">
                    <div class="p-5">
                        <a href="#">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900">{{$course->course_name}}</h5>
                        </a>
                        <p class="font-normal text-gray-700">Start {{ $course->start_date }}</p>
                        <p class="font-normal text-gray-700">Course By {{ $course->name }}</p>
                        ...
                        <a href="{{ route('admin-course-detail', ['idcourse' => $course->id]) }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                            Read more
                        </a>
                        <a href="{{ route('admin-edit-course', ['idcourse' => $course->id]) }}" class="m-4 inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                            Edit
                        </a>
